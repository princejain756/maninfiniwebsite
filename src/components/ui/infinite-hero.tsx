	...
	return (
		<mesh ref={meshRef}>
			...
			{(() => {
				...
				return <shaderMaterial {...materialProps} />;
			})()}
		</mesh>
	);
}
		...
		>
			...
			<div className="relative z-10 flex h-svh w-full items-center justify-center px-6">
				<div className="text-left max-w-5xl">
					<h1
						ref={h1Ref}
						className="mx-auto max-w-3xl lg:max-w-5xl text-[clamp(2.25rem,6vw,4rem)] font-extrabold leading-[0.95] tracking-tight"
					>
						End-to-End
					...
					<p
						ref={pRef}
						className="mx-auto mt-4 max-w-2xl md:text-balance text-base md:text-xl font-normal tracking-tight text-white/90"
					>
					...
					<div
						ref={ctaRef}
						className="mt-8 flex flex-col sm:flex-row items-start justify-start gap-4"
					>
						<button
							type="button"
							...
							className="group relative overflow-hidden border border-white/30 bg-gradient-to-r from-white/20 to-white/10 px-6 py-3 text-base rounded-lg font-medium tracking-wide text-white backdrop-blur-sm transition-[border-color,background-color,box-shadow] duration-500 hover:border-white/50 hover:bg-white/20 hover:shadow-lg hover:shadow-white/10 cursor-pointer flex items-center"
						>
						...
						<button
							...
							className="group relative px-4 py-2 text-sm font-medium tracking-wide text-white/90 transition-[filter,color] duration-500 hover:drop-shadow-[0_0_6px_rgba(255,255,255,0.6)] hover:text-white cursor-pointer inline-flex items-center gap-2"
						>
